...
module.exports = {
  config: {
    ...
    gccDebug: {
      ...
      description: "Please read the linter-gcc wiki [here](https://github.com/hebaishi/linter-gcc) before reporting any issues.",
      type: "boolean",
      default: false,
      order : 12
    },
    ...
    avrDevice: {
        title: "AVR MCU name",
        description: "AVR device name, ex: atmega128, attiny2313...",
        type: "string",
        default: "atmega128",
        order : 15
    },
    avrDeviceFrequency: {
        title: "AVR MCU frequency",
        description: "The frequency of AVR device, ex: 11059200, 8000000...",
        type: "string",
        default: "11059200",
        order : 16
    },
    avrgccDefaultCFlags: {
      title: "AVR-GCC Flags",
      description: "Supports the use of escaped characters, default is the settings in AVR studio4",
      type: "string",
      default: "-Wall -gdwarf-2 -std=gnu99 -Os -funsigned-char -funsigned-bitfields -fpack-struct -fshort-enums",
      order : 17
    //   -mmcu=atmega128 -Wall -gdwarf-2 -std=gnu99 -DF_CPU=11059200UL -Os -funsigned-char -funsigned-bitfields -fpack-struct -fshort-enums
    },
    avrcMode: {
      title: "AVR mode",
      description: "to lint with AVR-GCC",
      type: "boolean",
      default: "false",
      order : 18
    }
  ...
  lint: function(editor, linted_file, real_file){
    ...
    var regex
    if ( atom.config.get("linter-gcc-with-avr.avrcMode") ) {
        execPath = atom.config.get("linter-gcc-with-avr.exeAvrcPath");
        execPathArray = execPath.split('.');
        regex = `(?<file>.+):(?<line>\\d+):\\s*\\w*\\s*(?<type>(${atom.config.get("linter-gcc-with-avr.gccErrorString")}|${atom.config.get("linter-gcc-with-avr.gccWarningString")}|${atom.config.get("linter-gcc-with-avr.gccNoteString")})):\\s*(?<message>.*)`
    }
    else {
        execPath = atom.config.get("linter-gcc-with-avr.execPath");
        execPathArray = execPath.split('.');
        regex = `(?<file>.+):(?<line>\\d+):(?<col>\\d+):\\s*\\w*\\s*(?<type>(${atom.config.get("linter-gcc-with-avr.gccErrorString")}|${atom.config.get("linter-gcc-with-avr.gccWarningString")}|${atom.config.get("linter-gcc-with-avr.gccNoteString")})):\\s*(?<message>.*)`
    }
    ...
    return helpers.exec(command.binary, command.args, {stream: "stderr"}).then(output => {
      ...
      msgs.forEach(function(entry){
        ...
        if (entry.type ===`${atom.config.get("linter-gcc-with-avr.gccWarningString")}`)
            entry.type = "warning";
        else if (entry.type ===`${atom.config.get("linter-gcc-with-avr.gccErrorString")}`)
            entry.type = "error";
        else if (entry.type ===`${atom.config.get("linter-gcc-with-avr.gccNoteString")}`)
            entry.type = "note";
      })
  ...
  activate: function() {
    ...
    require("atom-package-deps").install("linter-gcc-with-avr");
    time_last_lint = new Date().getTime()
    lint_waiting = false
  },
  deactivate: function() {
  ...
  consumeLinter: function(indieRegistry) {
    ...
    lintOnTheFly = function() {
      ...
      if (!editor) return;
      if (atom.config.get("linter-gcc-with-avr.gccLintOnTheFly") == false) return;
      if (lint_waiting) return;
      lint_waiting = true
      interval = atom.config.get("linter-gcc-with-avr.gccLintOnTheFlyInterval")
    ...
    lintOnSave = function(){
      editor = utility.getValidEditor(atom.workspace.getActiveTextEditor());
      if (!editor) return;
      if (atom.config.get("linter-gcc-with-avr.gccLintOnTheFly") == true) return;
