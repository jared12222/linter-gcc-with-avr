module.exports = {
  ...
  buildCommand: function(activeEditor, file) {
    ...
    if (atom.config.get("linter-gcc-with-avr.gccDebug")){
      console.log("linter-gcc-with-avr config: " + JSON.stringify(settings));
    }
    ...
    if (settings.compileCommandsFile.trim() != "") {
      ...
      var exePath ="";
      if ( settings.avrcMode ) {
        exePath = settings.exeAvrcPath;
      }
      else {
        exePath = settings.execPath;
      }
      command = exePath;
      ...
      if (!command) {
        atom.notifications.addError(
          "linter-gcc-with-avr: Executable not found", {
            detail: "\"" + settings.execPath + "\" not found"
          }
        );
        console.log("linter-gcc-with-avr: \"" + settings.execPath + "\" not found");
        return;
      }
      ...
      if (grammar_type === "C++") {
        ...
        if ( atom.config.get("linter-gcc-with-avr.avrcMode") ) {
            flags += "-mmcu=" + settings.avrDevice;
            flags += " -DF_CPU=" + settings.avrDeviceFrequency + "UL ";
            flags += settings.avrgccDefaultCFlags;
        }
        else {
    ...
    if (atom.config.get("linter-gcc-with-avr.gccDebug")){
      full_command = "linter-gcc-with-avr: " + command;
